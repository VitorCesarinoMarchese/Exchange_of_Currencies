...
function TransactionHistory({
  ...
  return (
    <div className="py-4">
      <h2 className="font-bold text-3xl self-start p-2">{t("Transaction History")}</h2>
      <div className="max-w-[343px] md:max-w-[675px]">
        <table
          className="border-collapse w-full table-fixed rounded-lg overflow-hidden"
          key={history.length}
        >
          <thead>
          ...
          <tbody className="border">
            {loading ? (
              <tr className="border-b hover:bg-gray-100">
                <td className="px-4 py-2 text-xs md:text-lg truncate">
                  {t("Loading")} ...
                </td>
                <td className="px-4 py-2 text-xs md:text-lg truncate">
                  {t("Loading")} ...
              ...
              paginatedHistory.map((row) => {
                const { formattedDate, formattedYear } = formatDate(
        ...
        {location == "/history" ? (
          <div className="flex justify-center items-center mt-4">
            ...
            <span className="px-3">
              Page {currentPage} of{" "}
              {Math.ceil(sortHistory.length / itemsPerPage) || 1}
            </span>
            <Btn
              label="Next"
              color={`${
                currentPage >= Math.ceil(sortHistory.length / itemsPerPage)
                  ? "gray-400"
                  : "white"
              }`}
              w="w-fit"
              ...
              func={() =>
                setCurrentPage((prev) =>
                  Math.min(
                    prev + 1,
                    Math.ceil(sortHistory.length / itemsPerPage)
                  )
                )
              }
              disable={
